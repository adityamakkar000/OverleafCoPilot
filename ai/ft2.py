...
def main(cfg: ftConfig) -> None:
    ...
    if os.path.exists(f"{cfg.output}/{cfg.name}/config.yaml"):
        if cfg.overwrite:
            ...
            raise FileExistsError(
                "The output directory already exists. Please set overwrite to true to overwrite the directory."
            )
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        model_id, add_eos_token=True, padding_side="left"
    )
    ...
    lora_config = LoraConfig(
        ...
        if len(modules) < modules_limit
        else modules[:modules_limit],
        lora_dropout=0.05,
        bias="none",
        task_type="CAUSAL_LM",
    )
